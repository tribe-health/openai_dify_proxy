...
use crate::features::dify::handlers::openai::image::{
...
async fn test_create_image_timeout() {
    // Setup mock database with delayed response
    let mut mock_db = MockDbClient::new();
    mock_db.expect_create_image_job()
        .returning(|_| {
...
async fn test_webhook_failure() {
    ...
    // This would require additional mock expectations
}

// Helper function to create a test image request
fn create_test_request() -> OpenAIImageRequest {
    OpenAIImageRequest {
...
// Helper function to create a test webhook payload
fn create_test_webhook_payload(status: &str) -> ReplicateWebhookPayload {
